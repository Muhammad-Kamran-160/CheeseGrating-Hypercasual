...
public static class SimplePool {
	...
	class Pool {
		...
		public GameObject SpawnWithDefaultRotation(Vector3 pos) {
			GameObject obj;
			if(inactive.Count==0) {
				// We don't have an object in our pool, so we
				// instantiate a whole new object.
				obj = (GameObject)GameObject.Instantiate(prefab, pos, prefab.transform.rotation);
				obj.name = prefab.name + " ("+(nextId++)+")";
				...
				if(obj == null) {
					...
					return SpawnWithDefaultRotation(pos);
				}
			}
	...
	static public void Preload(GameObject prefab, int qty = 1) {
		...
		for (int i = 0; i < qty; i++) {
			obs[i] = Spawn (prefab, Vector3.zero, prefab.transform.rotation);
		}
	...
	static public GameObject SpawnWithDefaultRotation(GameObject prefab, Vector3 pos) {
		Init(prefab);

		return pools[prefab].SpawnWithDefaultRotation(pos);
	}
